...
공통
- ICMP 차단 시 PMTUD(Path MTU Discovery) 실패
- - PMTUD: 패킷을 쪼개지 않고, 목적지까지 갈 수 있는 최대 MTU 크기를 ICMP 메시지로 자동 탐색하는 기법
- TCP 정체 발생

대응
- MSS Clamp
- - TCP SYN 패킷의 MSS 값을 강제로 줄여, MTU 문제로 인한 단절을 사전에 차단하는 기법
- PLPMTUD(Packetization Layer Path MTU Discovery)
- - ICMP에 의존하지 않고, 전송 계층(TCP/UDP)이 직접 패킷 손실을 관측해 MTU를 탐색하는 방식
